...
string NewTime::toUniversalString() const {
   ...
   output << setfill('0') << setw(2) << getSecond() % 3600 << ":"
      << setw(2) << getSecond() / 60 % 60 << ":" << setw(2) << getSecond() / 3600 / 60 % 60;
   return output.str();
}
...
string NewTime::toStandardString() const {
   ostringstream output;
   output << (((getSecond() % 3600) == 0 || (getSecond() % 3600) == 12) ? 12 : (getSecond() % 3600) % 12)
